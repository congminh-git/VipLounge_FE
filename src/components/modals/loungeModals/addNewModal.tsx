...
import { addLounge } from '@/services/lounge';
...
export default function AddNewModal({ isOpen, onOpenChange, reFetchData, listAirports, listAgencies }: IAddNewModal) {
    ...
    const create = async (body: ICreate) => {
        const result = await addLounge(body);
        return result;
    };
    ...
    const onSubmitModal = async (name: string, code: string, agencyCode: string, airportCode: string) => {
        ...
        if (!nameInvalidBool && !codeInvalidBool && !airportCodeInvalidBool && !airportCodeInvalidBool) {
            ...
            if (result.id) {
                Swal.fire({
                    title: 'Thêm phòng chờ thành công',
                    icon: 'success',
        ...
        <>
            ...
            >
                <ModalContent>
                    {(onClose) => (
                        <>
                            <ModalHeader className="flex flex-col gap-1">Thêm phòng chờ</ModalHeader>
                            <ModalBody>
                                <Input
                                    size={'sm'}
                                    type="text"
                                ...
                                <Input
                                    ...
                                    errorMessage="Mã phòng chờ phải có 3 ký tự"
                                    value={code}
